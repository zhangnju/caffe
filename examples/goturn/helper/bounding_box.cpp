...
BoundingBox::BoundingBox(const vot_region& region)
{
  // VOTRegion is given by left, top, width, and height.
  x1_ = region.x;
  y1_ = region.y;
  x2_ = region.x + region.width;
  y2_ = region.y + region.height;
}

void BoundingBox::GetRegion(vot_region* region) {
  // VOTRegion is given by left, top, width, and height.
  region->x=x1_;
  region->y=y1_;
  region->width=get_width();
  region->height=get_height();
}
